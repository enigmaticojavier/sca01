...
public class ProcesoProyecto {
    ...
    public BeanRetornoData procesar(int personaId, String periodo, List lstProceso){
        ...
        try{
            ...
            cs = conn.prepareCall("{call " + ConstantesSistema.PROC_CARGA_PROYECTO + "(?,?,?)}");
            ...
            if (resProc == -2){
                beanRetornoData.setCodError(ConstantesSistema.CONST_RETORNO_ERROR_MSG); 
                beanRetornoData.setDscError("Error ejecutando procedimiento " + ConstantesSistema.PROC_CARGA_PROYECTO );
                return beanRetornoData;
            }
